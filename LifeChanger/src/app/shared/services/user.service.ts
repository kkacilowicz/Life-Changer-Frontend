...
export class UserService {
  ...
  getPreferences(): Observable<userPreferences> {
    ...
    return this.httpClient.get<userPreferences>(
      this.preUrl + 'UserCategories',
      { headers: headers }
    );
  }
  ...
  preferences(): Promise<void> {
    ...
    this.getPreferences().subscribe((data: userPreferences) => {
      this.userPref = {
  ...
  updateUser(model: any) {
    ...
    return this.httpClient
      .patch(this.userUrl + 'updateuser', model, options)
      .pipe(
        map((user: any) => {
          if (user) {
